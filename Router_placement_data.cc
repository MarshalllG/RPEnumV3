...
bool RP_Output::ThereAreRoutersInNeighborhood (point p1, point p2) const
{
    for (unsigned r=min(p1.row, p2.row); r <= max (p1.row, p2.row); r++)
    {
        for (unsigned c=min(p1.col, p2.col); c <= max (p1.col, p2.col); c++)
        {
            if (out_cells[r][c] == 'r' && r != p2.row && c != p2.col)
                return 1;
        }
    }
    return 0;
...
// dato un punto, restituisce che celle coprirebbe se fosse un router.
vector<point> RP_Output::FindAreaToUncover(point router) const
{
    point temp;
    unsigned step = 1;
    vector<point> area;
        ...
        for (int r=router.row-step; r <= router.row+step; r++)
        {
            for (int c=router.col-step; c <= router.col+step; c++)
            {
                if (in.IsGridPoint(r,c))
                {
                    temp.row = r;
                    temp.col = c;
                    if (out_cells[r][c] == '1' || out_cells[r][c] == 'r' && !ThereAreRoutersInNeighborhood(temp, router))
                    {
                        if (InfDistance(temp, router) == step)
                            area.push_back(temp);
                    }
